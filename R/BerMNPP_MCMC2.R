BerMNPP_MCMC2 <- function(n0,y0,n,y,prior_p,prior_delta_alpha,prior_delta_beta,prop_delta_alpha,
  ...
  LogPostBerdelta = function(x){
    ...
    out = lbeta(deltay0+y+k*(prior_p[1]-1)+1,n-y+deltanmy0+k*(prior_p[2]-1)+1)- 
      sum(lbeta(x*y0+prior_p[1],x*(n0-y0)+prior_p[2]))+ 
  ...
  s1 = delta_sample%*%y0 + y + k*(prior_p[1]-1)+1
  s2 = delta_sample%*%(n0-y0) + n - y + k*(prior_p[2]-1)+1
